...
const ReportServiceMock = vi.mocked(ReportService);
...
describe('SupervisorView', () => {
  ...
  it('should display an error message if fetching fails', async () => {
    ReportServiceMock.getSupervisorReport.mockRejectedValue(
      new Error('API Error')
    );
    renderComponent();
    expect(
  ...
  it('should display a message when there is no report data', async () => {
    ...
    renderComponent();
    expect(
      await screen.findByText('No pending training for your team members.')
    ).toBeInTheDocument();
  });
  ...
  it('should render the report table with data', async () => {
    ...
    const memberHeaders = await screen.findAllByRole('columnheader', {
      name: 'Member',
    });
